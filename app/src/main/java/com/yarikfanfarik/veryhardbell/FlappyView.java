...
public class FlappyView extends View{
 ...
 public FlappyView(Context c){super(c);for(int i=0;i<10;i++){xs.add(700+i*320f);gaps.add(120+r.nextInt(320));}last=System.currentTimeMillis();}
 ...
 @Override public boolean onTouchEvent(android.view.MotionEvent e){if(e.getAction()==MotionEvent.ACTION_DOWN){if(over){birdY=getHeight()/2f;vel=0;over=false;for(int i=0;i<10;i++){xs.set(i,700+i*320f);gaps.set(i,120+r.nextInt(320));}last=System.currentTimeMillis();postInvalidate();}else vel=-350;return true;}return true;}
}
